    ...
    def _send_email_notifications(self, emails):
        for email in emails:
            try:
                ...
                print({"message": f"Email Sent Successfully to {email}"})
            except Exception as e:
                print({"Error": f"Error sending email to {email}: {str(e)}"})
                    ...
                    if valid_messages:
                        for message in valid_messages:
                            if message.get("status") == "queued":
                                print(
                                    {
                                        "message": f"SMS Sent Successfully to {number} in queue"
                                    }
                                )
                                break
                else:
                    print({"message": f"Failed to send SMS to {number}: {response}"})
            except Exception as e:
                print({"Error": f"Error sending SMS to {number}: {str(e)}"})
