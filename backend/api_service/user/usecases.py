...
class LoginUseCase(BaseUseCase):
    ...
    def _register_device(self, user):
        ...
        existing_token = OrganizationFCMToken.objects.filter(
            organization=user, fcm_token=fcm_token
        ).first()
        if existing_token:
            return

        OrganizationFCMToken.objects.create(organization=user, fcm_token=fcm_token)
...
class ForgetPasswordUseCase(BaseUseCase):
    ...
    def _factory(self):
        ...
        try:
            ...
            raise ValidationError(
                {"error": "User with this  mobile number does not exist in the system"}
            )
        ...
        notification_data = {"title": "Reset Password", "message": otp_message}
...
class UserUpdateUseCase(BaseUseCase):
    ...
    def _is_valid(self):
        if CustomUser.objects.filter(
            mobile_number=self._data["mobile_number"]
        ).exists():
            raise ValidationError(
