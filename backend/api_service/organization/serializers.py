...
class CreateOrganizationKycSerializer(serializers.ModelSerializer):
    social_media_links = OrganizationSocialMediaLinkSerializer(
...
class OrganizationListKYCSerializer(serializers.ModelSerializer):
    ...
    def get_registration_certificate(self, obj):
        if obj.registration_certificate and hasattr(
    ...
    def get_PAN_VAT_certificate(self, obj):
        if obj.PAN_VAT_certificate and hasattr(obj.PAN_VAT_certificate, "url"):
            parsed_url = urlsplit(obj.PAN_VAT_certificate.url)
            return parsed_url.path
        return None

    def get_licenses(self, obj):
        if obj.licenses and hasattr(obj.licenses, "url"):
            parsed_url = urlsplit(obj.licenses.url)
            return parsed_url.path
        return None

    def get_citizenship(self, obj):
        if obj.citizenship and hasattr(obj.citizenship, "url"):
    ...
    def get_driving_license(self, obj):
        if obj.driving_license and hasattr(obj.driving_license, "url"):
