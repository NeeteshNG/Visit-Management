...
def send_sms(to, text):
    try:
        ...
        if response_json.get("status") == "queued":
            print(f"SMS Sent Successfully to {to} in queue.")
            return response.status_code, response.text, response_json
        else:
            print(f"Failed to send SMS to {to}: {response_json}")
            return response.status_code, response.text, response_json
...
def send_notification(user, notification_data):
    NotificationData.objects.create(user_id=user, **notification_data)
    ...
    try:
        ...
        print({"message": f"Email Sent Successfully to {recipient_email}"})
    except Exception as e:
        print({"Error": f"Error sending email to {recipient_email}: {str(e)}"})
    ...
    try:
        ...
        if status_code == 200:
            valid_messages = response_json.get("data", {}).get("valid", [])
            if valid_messages:
                for message in valid_messages:
                    if message.get("status") == "queued":
                        print(
                            {
                                "message": f"SMS Sent Successfully to {recipient_mobile} in queue"
                            }
                        )
                        break
        else:
            print({"message": f"Failed to send SMS to {recipient_mobile}: {response}"})
    except Exception as e:
        print({"Error": f"Error sending SMS to {recipient_mobile}: {str(e)}"})
