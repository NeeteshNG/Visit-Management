...
class OrganizationVisitStatisticsView(View):
    def get(self, request, organization_id):
        ...
        visit_stats_dict = {stat["truncated_date"]: stat for stat in visit_stats}

        all_dates = [
            start_date + datetime.timedelta(days=x)
            for x in range((end_date - start_date).days + 1)
        ] if start_date_str and end_date_str else []
        ...
        response_data = [
            {
                "label": day_mapping.get(date.weekday(), "Unknown"),
                "date": date.strftime("%Y-%m-%d"),
                "check_in": visit_stats_dict.get(date, {}).get("check_in", 0),
                "check_out": visit_stats_dict.get(date, {}).get("check_out", 0),
                "totalvisit": visit_stats_dict.get(date, {}).get("total_visit", 0),
            }
            for date in all_dates
        ] if all_dates else [
            {
                "label": day_mapping.get(stat["truncated_date"].weekday(), "Unknown"),
                "date": stat["truncated_date"].strftime("%Y-%m-%d"),
                "check_in": stat["check_in"],
                "check_out": stat["check_out"],
                "totalvisit": stat["total_visit"],
            }
...
class OrganizationHistoryVisitorCountView(generics.ListAPIView):
    ...
    def get_object(self):
        ...
        if not organization:
            raise ValidationError(
                {"error": "Organization does not exist for the given id."}
            )
        return organization
    ...
    def get_queryset(self):
        organization = self.get_object()
        visitor_counts = (
            OrganizationVisitHistory.objects.filter(organization=organization)
            .values("visit_type")
        ...
        return [
            ...
            {"visit_type": "Total", "count": total_count},
        ]
